...
int get_code(uint32_t *code_addr) {
    ...
    uint32_t ndatabytes = boot_get32();
    uint32_t npayloads = boot_get32(); //might be redundant. remove from both sides if so.
    uint32_t cksum_data = boot_get32();
    ...
    //final proj: and also check that adding ndatabytes is also OK
    if ( (uint32_t)addr + nbytes + ndatabytes >= (uint32_t)&PUT32) {
    ...
    if (crc32(addr, nbytes) != cksum) {
        boot_err(BOOT_ERROR, "code cksum wrong!\n");
    }

    //FINAL PROJ: ONLY DO THIS for when there are dependencies!!
    //^ if the values are zero then there are no dependencies:
    //note: .bins themselves know their dependencies so this is fine
    if (ndatabytes != 0 && npayloads != 0) {
    	//final project: request data now!
    	//final proj: send data checksum for checking now too
    	boot_put32(GET_DATA);
    	boot_put32(cksum_data);

    	//final proj: expect PUT_DATA then the data
    	//final proj: read each byte and write starting at __data_start__
    	//this is the part where we might consider writing to SD or soemthing
    	while ( (boot_got = boot_get32()) != PUT_DATA ) {}
    	char *put_data_addr = (char *)__data_start__;
	//printk("ds is %x\n", __data_start__);
    	for (int i = 0; i < ndatabytes; i++) {
    	    uint8_t boot_got8 = uart_get8();
    	    PUT8((uint32_t)put_data_addr, boot_got8);
    	    put_data_addr++;
    	}

    	//final proj: very cksum of copied data just like above for code
    	if (crc32((char *)__data_start__, ndatabytes) != cksum_data) {
    	    boot_err(BOOT_ERROR, "data cksum wrong!\n");
    	}
    }

    // 7. send back a BOOT_SUCCESS!
    boot_putk("UART Teddy Zhang: success: Received the program and dependency data!\n");
