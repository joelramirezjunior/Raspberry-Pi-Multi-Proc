...
static void usage(const char *msg, ...) {
    ...
    output("       --last_n: gets the last n serial device mounted\n");
...
int main(int argc, char *argv[]) { 
    ...
    for(int i = 1; i < argc; i++) {
        ...
        } else if (strcmp(argv[i], "--last_n") == 0) {
            dev_name = find_ttyusb_last_n();
        } else if(strcmp(argv[i], "--first") == 0)  {
            dev_names = find_ttyusb_first();
    ...
    if(!dev_names) {
        dev_names = find_ttyusb_last_n();
        if(!dev_name)
            panic("didn't find a device\n");
    }
    debug_output("done with options: dev names=<%s>, pi-prog=<%s>, trace=%d\n", 
            dev_names, pi_prog, trace_p);
