...
void sw_uart_put8(sw_uart_t *uart, unsigned char c) {
    ...
    unsigned t = cycle_cnt_read();
    uint8_t pin = uart->tx;
    uint32_t ncycles = uart->cycle_per_bit;
    write_cyc_until(pin, 0, t, ncycles); 
    t += ncycles;
    for (int i = 0; i < 8; i++) {
        unsigned val = (c >> i) & 0x1; 
        write_cyc_until(pin, val, t, ncycles); 
        t += ncycles;
    }

    write_cyc_until(pin, 1, t, ncycles);
